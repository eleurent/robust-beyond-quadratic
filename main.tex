...
Despite the dazzling successes of Reinforcement learning in cracking many problems considered unsolvable in the past decade [ref needed], this tool has hardly been applied in real industrial issues. This could be attributed to two undesirable properties which limit its practical applications: its dependence on a tremendous amount of interaction data that cannot always be simulated, and its reliance on trial-and-error and random exploration. In this paper, we consider the problem of controlling an unknown system in a critical setting where mistakes are costly and must be avoided at all time. Crucially, we wish to handle \emph{arbitrary} bounded reward functions $R$, for instance non-convex and non-smooth functions that naturally arise in problems 
...
\begin{algorithm}[tb]
	\caption{Robust Estimation, Prediction and Control (v2)}
	\begin{algorithmic}
		\STATE {\bfseries Input:} confidence level $\delta$
		\FOR{$N = 1,2,3,\dots$} 
