...
\paragraph{Experiments} Finally, our last contribution demonstrates the applicability of our proposed \autoref{alg:full} in two numerical experiments, a simple illustrative example and a more challenging simulation for an autonomous driving application.
...
\|\theta_{N,\lambda} - \theta\|_1 \leq {} &\|G_{N, \lambda}^{-1}\sum_{n=1}^N \Phi_n^\transp \Sigma_p^{-1}\|_1\|\eta\|_\infty + \lambda\|G_{N, \lambda}^{-1}\|_1 S
\end{split}
\end{equation}
...
We recover \eqref{eq:lyap-bound} with $\mu = \frac{\lambda_{\min}(Q)}{\lambda_{\max}(P)} = \frac{\frac{1}{2}\lambda_{\min}(Q_0) - 4\alpha}{\lambda_{\max}(P)}$.
Finally, we obtain
\begin{align*}
