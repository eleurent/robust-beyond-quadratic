...
One of the major benefits of using the family of linear models, compared to richer model classes, is that it enables to measure empirically the adequacy of the modelling assumptions to capture the observed signal, and thus to raise an alert when the adequacy is too low.

We define the model adequacy for an observation $y$ at point $x$ of the modelling assumptions $(A, \phi)$ trained on history $\cD$ by:
\begin{equation*}
    \alpha(y,x)
\end{equation*}
