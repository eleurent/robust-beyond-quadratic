...
$
    \dot{x}(t) + C\nu(t)
$ where $\nu(t)\in\Real^r$ is a measurement noise and $C\in\Real^{p\times r}$ is known.
For better readability, we will actually denote the measurement as:
\begin{equation}
\label{eq:measurement}
    y(t) = \dot{x}(t) - A_0 x(t) - Bu(t) + C\nu(t)
\end{equation}
which is possible since the nominal dynamics $A_0$, thet state $x(t)$ and the control $u(t)$ are known.
Assumptions over the disturbance $\omega$ and noise $\nu$ will be detailed further, and we denote their total contribution as $\eta(t) = C\nu(t) + D\omega(t)$.
...
With Hölder inequality, we obtain a confidence region $\cC$ defined by a $L_1$-ball, i.e. a polytope with $2d$ vertices:
\begin{equation}
\begin{split}
