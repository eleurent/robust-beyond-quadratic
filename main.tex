...
\begin{abstract}
We develop a framework for the robust adaptive model predictive control (MPC) of a linear system with unknown parameters, in a critical setting where failures are costly. So far, this problem has been studied in the case of quadratic costs --the LQ problem--, which limits applications to stabilisation tasks. In this paper, we extend this setting to consider arbitrary bounded costs, including non-convex, that arise in many practical problems. To this end, we carefully select and bring together several tools from different communities, namely linear regression, recent results in interval prediction, and tree-based planning. Thanks to the non-trivial combination of theoretical guarantees at each layer, we provide the first end-to-end regret analysis in this setting. Finally, we relax our modelling assumptions in a multi-model extension based on a data-driven robust model selection mechanism. In addition to our theoretical results we strived to preserve tractable computations for practical applicability, as we illustrate on a two simulated applications.\footnote{Source code and videos are available in the Supplementary Material.}
\end{abstract}	
