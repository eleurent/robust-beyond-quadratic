...
From now on, we consider that we have reached step $N$, and that a confidence region $\cC_{[N],\delta}$ was obtained with data $\cD_{[N]}$ collected so far. The following sections tackle the optimisation of the robust control objective:
\begin{equation}
\label{eq:robust-control}
\max_\bu \underbrace{\min_{\substack{A(\theta) \in \cC_{[N],\delta} \\ \bw\in[\underline{\bw},\overline{\bw}]^\Natural}} \left[\sum_{n=1}^\infty \gamma^n R(x_n(\bu,\bw))\right]}_{V^r(\bu)}
\end{equation}
