...
\begin{theorem}[Approximation gap]
\label{thm:control-error}
If the reward function $R$ is lipschitz, then we can bound the approximation error of \autoref{alg:full}:
\begin{equation*}
\hat{V}^r(\bu) \leq {V}^r(\bu) \leq V(\bu) \leq \hat{V}^r(\bu) + \cO\left(\frac{1}{\sqrt{N}}\right)
\end{equation*}
\end{theorem}
...
\begin{table}[htbp]
	...
	\begin{tabular}{lcccccc}
		...
		\multicolumn{3}{c}{Obstacle}    &
		\multicolumn{3}{c}{Driving}\\
		\cmidrule(lr){2-4}
		\cmidrule(lr){5-7}
		Perf. &
		\multicolumn{1}{c}{min} &
		...
		\multicolumn{1}{c}{failures}\\
		\midrule
		Oracle & {11.6} & {14.2} & 0\% & {0} & {0} & 0\\
		{Nominal} & {2.8} & \textbf{13.8} & 4\% & {0} & {0} & 0 \\
		Robust & \textbf{10.4} & {13.0} & \textbf{0\%} & {0} & {0} & 0 \\
		\bottomrule
	\end{tabular}
\end{table}
...
&\leq \lambda_{\max}(\phi_{i}^{\transp}\phi_j) \|\theta - \theta_{[N],\lambda}\|_{2} = \cO\left( \sqrt{\frac{\log N}{N}} \right) 
\end{align*}
...
\[\|\ox(t) - \ux(t)\| \leq C_\omega(t) +\cO\left( \sqrt{\frac{\log N}{N}} \right) \]
where $C_\omega(t)$ is the interval size $\|\ox(t) - \ux(t)\|$ obtained by running \eqref{eq:interval-predictor} with $\Delta A=0$.
...
Moreover, it holds that $-\ux^+ -\ox^- \leq e \leq \ox^+ + \ux^-$, which implies $|\ux^+ + \ox^-| \leq 2 |e|$.
...
e\\ \ux^+ + \ox^- \\ \overline{\omega} - \underline{\omega}
\end{bmatrix}$. Hence, 
\begin{align*}
\dot{V} \leq {}& X^\transp
...
X\\
& - e^\transp Q e + \rho |\overline{\omega} - \underline{\omega}|^2
\end{align*}

Thus, if $\Upsilon < 0$, $Q>0$, $\rho > 0$, $V$ is bounded and can only increases from $V(e(t_N)) = 0$ to a $V(e)$ such that:
\[
e^\transp Q e = X^\transp \Upsilon X + \rho |\overline{\omega} - \underline{\omega}|^2
\]
\end{proof}
...
\begin{proof}
 ...
 For any sequence of controls $\bu$, dynamics $A(\theta)\in C_{[N],\delta}$ and perturbations $\underline{\bw} \leq \bw \leq \overline{\bw}$, we clearly have 
 \[V(\bu)^r \leq V(\bu) = \expectedvalue_{\bw}\sum_n \gamma^n R(x_n)\]
 ...
 \begin{align}
     ...
     &\leq L\sum_{n>N} \gamma^n C_{\omega,n} + \cO\left(\sqrt{\log N / N}\right))
 \end{align}
\end{proof}
