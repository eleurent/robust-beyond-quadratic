...
This bound contains an input-dependent \hlb{estimation error} term, which can be further bounded under an additional assumption that the features are sufficiently excited. 
...
\begin{corollary}[Asymptotic near-optimality]
	\label{cor:pe}
		...
		\end{align} the stability condition 2. of \Cref{thm:minimax-regret-bound} can be relaxed to its limit: there exist $P,Q_0,\rho$ such that
		$$\begin{bmatrix}
...
We also study the evolution of the simple regret $V(x_N) - \hat{V}^r(x_N)$ with respect to the number of samples $N$, by comparing the empirical returns from a state $x_N$ to the value $V(x_N)$ that the agent would get by acting optimally from $x_N$ with knowledge of the dynamics. Although the assumptions of \Cref{thm:minimax-regret-bound} are not satisfied (e.g. non-smooth reward), the mean regret of the robust agent, shown in \Cref{fig:regret}, still decreases polynomially with $N$: \Cref{alg:full} gets \emph{more efficient} as it is \emph{more confident} while \emph{ensuring safety} at all time. In contrast, the nominal agent has a smaller average regret but collides with obstacles in $4\%$ of simulations. 
...
\paragraph{Motion planning for an autonomous vehicle}
...
\begin{proof}
	The predictor designed in \Cref{sec:prediction} verifies the inclusion property \eqref{eq:inclusion-property}. Thus, for sequence of controls $\bu$, any dynamics $A(\theta)\in C_{N,\delta}$, and disturbances $\underline{\bom} \leq \bom \leq \overline{\bom}$, the corresponding state at time $t_n$ is bounded by $\underline{x}_n \leq x_n \leq \overline{x}_n$, which implies that $R(x_n) \geq \min_{x\in[\underline{x}_n(\bu), \overline{x}_n(\bu)]}  R(x) = \underline{R}_n(\bu)$.
...
\begin{proof}
	...
	\dot{V} ={}& X^\transp
	\begin{bmatrix}
	A_N^\transp P + P A_N^\transp + Q & P|D| & P|\Delta A| \\
	|D|^\transp P & -\rho I_r & 0\\
	...
	\underbrace{
		\left[
		\begin{array}{cc|c}
		A_N^\transp P + P A_N^\transp + Q + 4\alpha I_p & P|D| & P|\Delta A| \\
		|D|^\transp P & -\rho I_r & 0\\
		\hline
		|\Delta A|^\transp P & 0 & -\alpha I_p
		\end{array}
	...
	\[R = \begin{bmatrix}
	-A_N^\transp P - P A_N^\transp - Q - 4\alpha I_p & -P|D|\\
	-|D|^\transp P & \rho I_r\\
	\end{bmatrix}\]
