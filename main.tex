...
% \end{equation}
% where $\Phi_{[N],i}\in\Real^{N\times p}$ is the $i^\text{th}$ component of $\Phi_{[N]}$ along the last dimension $i\in[d]$, and
% \begin{equation*}
...
%     &= \inp{(Y_{-i} - \theta_i\Phi_{i})^T}{(Y_{-i} - \theta_i\Phi_{i})^T}_{F,\Sigma_p^{-1}} + \lambda\|\theta\|_{\Sigma_d}^2\\
%     &= \theta_i^2\inp{\Phi_{i}^T}{\Phi_{i}^T}_{F,\Sigma_p^{-1}}
%     -\theta_i\inp{\Phi_{i}^T}{Y_{-i}^T}_{F,\Sigma_p^{-1}}
%     -\theta_i\inp{Y_{-i}^T}{\Phi_{i}^T}_{F,\Sigma_p^{-1}}
%     + \inp{Y_{-i}^T}{Y_{-i}^T}_{F,\Sigma_p^{-1}}
%     \\
...
% Hence, 
% \begin{align*}
%     \nabla_{\theta_i} J(\theta) &= 2\theta_i\inp{\Phi_{i}^T}{\Phi_{i}^T}_{F,\Sigma_p^{-1}}
%     -2\inp{\Phi_{i}^T}{Y_{-i}^T}_{F,\Sigma_p^{-1}}
%     +2\lambda\theta^T\Sigma_{d,i}\\
%     &= 2\theta_i\inp{\Phi_{i}^T}{\Phi_{i}^T}_{F,\Sigma_p^{-1}}
%     +2\sum_{j\neq i}\theta_j\inp{\Phi_{i}^T}{\Phi_{j}^T}_{F,\Sigma_p^{-1}}
%     -2\inp{\Phi_{i}^T}{Y^T}_{F,\Sigma_p^{-1}}
%     +2\lambda\theta^T\Sigma_{d,i}\\
...
%      = \inp{\Phi_{i}^T}{Y^T}_{F,\Sigma_p^{-1}}
% \end{align*}
...
% \todo[inline]{EL: The first term is a $\Real^d$ vector whose coefficients are inner product of matrices (a sum along both the time $N$ and state dimension $p$). How to analyze its concentration? For comparison, in \citep{Abbasi2011} $X^T\eta$ was a $\Real^d$ vector whose coefficients were inner product of vectors $(X_i^T\eta)_i\in[d]$ (a sum over time $N$)}

% \begin{lemma}[Theorem 1 of \citep{Abbasi2011}, Matrix version]
% Let $\{F_t\}_{t=0}$ be a filtration.
% Let $\{\eta_t\}_{t=1}^\infty$ be a $\Real^p$-valued stochastic process such that $\eta_t$ is $F_t$-measurable and $\eta_t$ is Gaussian with mean $0_p$ and covariance $\Sigma_p$.

% Let $\{X_t\}_{t=1}^\infty$ be an $\Real^{p\times d}$-valued stochastic process such that $X_t$ is $F_t$-measurable. Assume that $V$ is a $d\times d$ positive definite matrix. For any $t\geq 0$, define:

% \begin{equation*}
%     V_t = V + \sum_{s=1}^t X_s X_s^t \qquad S_t = \sum_{s=1}^t X_s^T\eta_s
% \end{equation*}
% Then, for any $\delta>0$, with probability at least $1-\delta$, for all $t\geq0$
% \todo[inline]{EL: Which kind of concentration?}
% \end{lemma}
% \begin{proof}
% \todo[inline]{TODO}
% \end{proof}
