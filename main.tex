...
This choice of rich reward space is crucial to have sufficient flexibility to model non-convex and non-smooth functions that naturally arise in many practical problems involving combinatorial optimisation, branching decisions, etc., while quadratic costs are mostly suited for the tracking of a reference trajectory already known a priori.

Since experiencing failures is out of question, the only way to prevent them from the outset, when the system is first deployed, is to rely on some sort of prior knowledge. In this work, we assume that the system dynamics are partially known, in the form of a parametrized dynamical system with unknown parameters. We argue that this assumption is realistic given that most industrial applications to date have been relying on physical models to describe their processes and well-engineered controllers to operate them, rather than machine learning. Our framework relaxes this modelling effort by allowing some structured uncertainty around the nominal model. We adopt a data-driven scheme where we wish to estimate the parameters more accurately as we interact with the true system. Most model-based reinforcement learning algorithms only rely on the estimated dynamics to derive the corresponding optimal controller, but these methods often suffer from \emph{model bias}: they ignore the error that exists between the learned and true dynamics, which can dramatically degrade the control performances \citep{Schneider1997}. This is all the more relevant when maximising an objective under constraints --or penalties-- which naturally pushes the systems to operate in the vicinity of constraint saturation and makes it prone to failures.

To address this issue, we turn to the framework of \emph{robust} decision-making: instead of merely considering a point estimate of the dynamics, we build an entire \emph{confidence region} that contains the true dynamics with high probability, and optimise the decisions so as to maximise the worst-case outcome with respect to this uncertainty. Minimax problem are hard to solve, and all the more in our setting where the reward function $R$ is generic. In particular, this excludes a number of tools from the robust synthesis literature, such as $\cH_\infty$ \citep[see, e.g.][]{Basar1996} control or system-level synthesis \citep{Dean2017,Dean2018}. We turn instead to more general planning algorithms. The continuous state space prevents us from using dynamic programming techniques, so we consider tree-based planning instead. Though there exist work addressing continuous actions \citep{Busoniu2018,Weinstein2012}, we resort to a first approximation and discretise the decision space by adopting a hierarchical control architecture. At each time, the agent can select low-level controller to execute among a finite basis, e.g. choose a short-term goal to reach or a skill to execute such as a driving maneuver. The discretisation induces a suboptimality, but it can be mitigated by diversifying the controller basis. Tree-based planning algorithms are relevant in this context of discrete action, but are only designed for a single known generative model instead of an entire confidence region for the system dynamics. Hence, we adapt them to the robust framework by following a two-steps procedure: we first predict a set of states that can be reached under a planned control sequence with respect to all possible dynamics. This reachability set is used to provide a conservative pessimistic reward to the planning algorithm. The next subsection aims to clarify and formalize these ideas, and state our contributions.
...
\end{equation}
 It is assumed that the parameter vector $\theta$ in the state matrix $A(\theta)\in\Real^{p\times p}$ belongs to a compact set $\Theta \subset \Real^d$. The control matrix $B\in\Real^{p\times q}$ and disturbance matrix $D\in\Real^{p\times r}$ are known.

We also assume having access to the observation of $x(t)$ and to a noisy measurement of $\dot{x}(t)$ in the form $y(t)=\dot{x}(t) + C\nu(t)$, where $\nu(t)\in\Real^s$ is a measurement noise and $C\in\Real^{p\times s}$ is known. Assumptions over the disturbance $\omega$ and noise $\nu$ will be detailed further, and we denote $\eta(t) = C\nu(t) + D\omega(t)$.
...
In \autoref{sec:control}, facing a sequential decision problem with continuous states and discrete actions, we turn to the literature of tree-based planning algorithms. Our third contribution adapts them to the robust objective \eqref{eq:robust-control-disc} by approximating it with an easily-evaluable surrogate $\hat{V}^r$ that exploits the set predictor \eqref{eq:inclusion-generic} to define a pessimistic reward. In our main result, we show that the best surrogate performance achieved during planning is guaranteed to be attained on the true system, and we provide an upper bound for the approximation gap and simple regret of our framework.

The \autoref{alg:full} shows the full integration of the three procedures of estimation, prediction and control. 

\paragraph{Model selection} In \autoref{sec:multi-model}, our forth contribution extends our framework to consider multiple modelling assumptions, while ensuring tight uncertainty estimation through data-driven model rejection, and safety via robust model-selection during planning.

\paragraph{Experiments} Finally, our last contribution demonstrates the benefits of our proposed \autoref{alg:full} in two numerical experiments, a simple illustrative example and a more challenging simulation for an autonomous driving application.
...
We abuse notations and include additional known terms in the measurement signal $y(t)$:
\begin{equation}
\label{eq:measurement}
    y(t) = \dot{x}(t) + C\nu(t) - A x(t) - Bu(t).
\end{equation}
Thus, we obtain a linear regression system
\[
...
\begin{align}
    \label{eq:polytope}
     &\cC_{[N],\delta} = \left\{ A_{0}+\sum_{i=1}^{2^d}\alpha_{i}\Delta A_{i}: \alpha\in[0, 1]^{2^d},  \sum_{i=1}^{2^d}\alpha_{i}=1\right\},\\
     &A_{0,N} = A + \theta_{N,\lambda}^\transp\phi, \quad\Delta A_{i} = \todo{h_i} \sqrt{\frac{\beta_N(\delta)}{\lambda_{\max}(G_{N,\lambda})}}
\end{align}
...
\begin{theorem}[Enhanced Predictor of \citealt{leurent2019interval}]
\label{thm:predictor}
...
This amounts to a change of reward function, except the worst case is assessed over the whole past trajectory, which means that this pessimistic reward $\underline{R_n}$ is not \emph{markovian}.
...
\end{equation*}
where $\Delta_\omega$ is a constant which corresponds to an irreductible suboptimality suffered from being robust to instantaneous perturbations $\omega(t)$.
\end{theorem}
...
The reward is non-smooth and encodes the task of reaching a goal state $x_g$ while avoiding collisions: $R(x) = \delta(x)/(1 + \|x - x_g\|_2)$  where $\delta(x)$ is $0$ whenever $x$ collides with an obstacle, $1$ otherwise. The actions $\cA$ are constant controls in the up, down, left and right directions.
...
This system dynamics $f$, which describes the couplings and interactions between vehicles, can only be expressed in the form of \autoref{assumpt:structure} given the knowledge of the desired routes of each vehicle. Then, the features $\phi$ express deviations of the vehicles trajectories to the centerline of the followed lane. Since these intentions are unknown to the agent, we adopt the multi-model perspective of \autoref{sec:multi-model} and consider one model per possible route for every observed vehicle before an intersection. Again, we compare \autoref{alg:full} to a nominal agent planning with the estimated parameter $\theta_{[N],\lambda}$, with two different modelling assumptions: we assume that Nominal 1 has access to the true followed route for each vehicle, while Nominal 2 does not and picks the model with minimal prediction error at the planning time. There performances are compared in \autoref{tab:driving}. Again, the robust approach is less efficient on average than planning with the nominal model, but it does better in terms of worst-case performance and manages to avoid collisions, contrary to the nominal baselines that suffer from model bias.
...
We resort to its Schur complement: $\Upsilon \leq 0$ if and only if $\alpha > 0$ and $R \geq S$, where $S= \alpha^{-1}\begin{bmatrix}|\Delta A|^\transp P & 0\end{bmatrix}^\transp \begin{bmatrix}|\Delta A|^\transp P & 0\end{bmatrix}$ and $R$ is the top-left block of $-\Upsilon$:
\[R = \begin{bmatrix}
...
Assume that $P$ is fixed and satisfies the conditions of the lemma. We have $\lambda_{\max}(S) \leq \alpha^{-1}\lambda_{\max}(P)^2\lambda_{\max}(|\Delta A|)^2$.
Thus, by taking $\alpha = \lambda_{\max}(|\Delta A|)^2 = \cO(\log N / N)$, $S = \begin{bmatrix}
S_0 & 0\\0 & 0
...
We recover \eqref{eq:lyap-bound} with $\mu = \frac{\lambda_{\min}(Q)}{\lambda_{\max}(P)} = \frac{\lambda_{\min}(Q_0) - \alpha}{\lambda_{\max}(P)}$.
Finally, we obtain
\begin{align*}
\|e(t)\|_2^2 &\leq \lambda_{\min}(P)^{-1} V(t)\\
& \leq \frac{\rho\lambda_{\max}(P)/\lambda_{\min}(P)}{\lambda_{\min}(Q_0) - 4\alpha} C_\omega(t)\\
\end{align*}
Developing at the first order in $\alpha_N$ gives
\begin{align*}
\|e(t)\|_2 &\leq C_0\left(1 + \frac{2\alpha}{\lambda_{\min}(Q_0)} + \cO(\alpha_N^2)\right)C_\omega(t)\\
