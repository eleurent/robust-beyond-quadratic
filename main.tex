...
\begin{abstract}
We develop a framework for the adaptive model predictive control (MPC) of a linear system with unknown parameters, in a critical setting where failures are costly and should be prevented at all time.  Through the careful selection and non-trivial combination of tools from different communities --namely linear regression, interval prediction and tree-based planning--, we provide an integrated solution to the problem of maximin control with arbitrary bounded costs. We ensure that theoretical guarantees at each layer could be combined to provide what we believe is the first end-to-end analysis in this general setting. The full procedure is designed to produce reasonable and safe behaviours, which progressively recover an asymptotic optimality. Finally, we relax our modelling assumptions in a multi-model extension based on a data-driven robust model selection mechanism. In addition to our theoretical results we strived to preserve tractable computations for practical applicability, as we illustrate on a simulated application of autonomous driving at a crossroads intersection.\footnote{Source code and videos are available in the Supplementary Material.}
\end{abstract}	
